...
class LeavingSequence:
    """Leaving animation that marches LEDs along the top and bottom rows.
    ...
    drive motors in sync with the LEDs during the leaving phase.
    """
    ...
    def __init__(
        ...
        if not self._addresses:
            warning("LeavingSequence created with no LED addresses")
        ...
        if not self._path:
            warning("LeavingSequence created with an empty path")

        # Simple motor leaving behavior configuration
    ...
    def start(self) -> None:
        """Start the leaving animation in a background thread."""
        if self._running:
            return

        if not self._path:
            warning("LeavingSequence cannot start because path is empty")
            return
        ...
        self._thread.start()
        self._running = True
        info("LeavingSequence started")

    def stop(self) -> None:
        """Stop the leaving animation and wait briefly for the thread to exit."""
    ...
    def _drive_motor_for_led(self, addr: LEDAddress) -> None:
        ...
        info(
            f"LeavingSequence: LED {addr} mapped to motor {motor_addr}. "
        ...
        try:
            ...
            warning(f"LeavingSequence: error issuing move_to for motor {motor_addr}: {e}")

    def _run_loop(self) -> None:
        """Worker loop that advances the leaving animation until stop is requested.
        ...
        try:
            if not self._path:
                warning("Leaving sequence run loop has empty path, exiting")
                return
            ...
            while not self._stop_event.is_set():
                for addr in self._path:
                    ...
                    try:
                        ...
                        warning(f"Leaving sequence tried to set unknown LED {addr}")
                        continue
                    except Exception as e:
                        error(f"Leaving sequence error while setting {addr} during fade in: {e}")
                        continue
                    ...
                    try:
                        ...
                        error(f"Leaving sequence error while setting {addr} during fade out: {e}")
            ...
            error(f"Leaving sequence run loop crashed: {e}")