...
class IdleSequence:
    """Background idle animation that marches LEDs along the top and bottom rows.

    This class owns a background thread that periodically calls
    LEDController.set_brightness while the sequence is running.
    """
    ...
    def __init__(
        ...
        next_led_delay: float = 0.8,
        max_brightness: int = 64,
        ...
        if not self._addresses:
            warning("LEDEngagedSequence created with no LED addresses")
        ...
        if not self._path:
            warning("LEDIdleSequence created with an empty path")

        # Simple motor idle behavior configuration
    ...
    def start(self) -> None:
        ...
        if not self._path:
            warning("LEDIdleSequence cannot start because path is empty")
            return
        ...
        self._thread.start()
        self._running = True
        info("LEDIdleSequence started")

    def stop(self) -> None:
        """Stop the idle animation and wait briefly for the thread to exit."""
    ...
    def _drive_motor_for_led(self, addr: LEDAddress) -> None:
        ...
        info(
            f"IdleSequence: LED {addr} mapped to motor {motor_addr}. "
        ...
        try:
            ...
            warning(f"IdleSequence: error issuing move_to for motor {motor_addr}: {e}")

    def _run_loop(self) -> None:
        """Worker loop that advances the animation until stop is requested.
        ...
        try:
            if not self._path:
                warning("Idle sequence run loop has empty path, exiting")
                return
            ...
            while not self._stop_event.is_set():
                for addr in self._path:
                    ...
                    try:
                        ...
                        warning(f"Idle sequence tried to set unknown LED {addr}")
                        continue
                    except Exception as e:
                        error(f"Idle sequence error while setting {addr} during fade in: {e}")
                        continue
                    ...
                    try:
                        ...
                        error(f"Idle sequence error while setting {addr} during fade out: {e}")
            ...
            error(f"Idle sequence run loop crashed: {e}")