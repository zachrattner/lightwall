...
// board name
static const char name[8] = "QUEBEC";
...
// non-blocking serial parser command buffer and indices
...
ISR(TCB0_INT_vect) {
  ...
  if (++tick20 >= TICKS_PER_MS) {
    tick20 = 0;
    // ms_accum++; commented out for now, pending finalization of firmware
...
static inline void blink_LED(uint8_t idx) {
  ...
  blink_timer_ms[idx] = 1000;     // duration in ms
  blink_active[idx] = 1;
