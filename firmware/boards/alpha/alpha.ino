...
// ===== Scheduler state =====
enum MotionMode : uint8_t { MODE_IDLE = 0, MODE_ROTATE, MODE_STEPS, MODE_POSITION };
...
static void controlModeSelect(uint8_t modeBits) {
  ...
  auto setModePin = [](uint8_t pin, bool highViaPullup) {
    if (highViaPullup) { pinMode(pin, INPUT); } // float HIGH via external pullup to ~3.3V
...
static inline void stop_motion(const char* reason) {
  ...
  stepsRemaining = 0;
  stepIntervalTicks = 0;
  Serial.print(F("STOP ")); Serial.println(reason);
}
...
static inline void step_once_and_bookkeep() {
  ...
  pulseClockEdge();
  if (dirParam == 0) { // CW
    currentStepIndex++; if (currentStepIndex >= STEPS_PER_REV) currentStepIndex -= STEPS_PER_REV;
  } else {
    currentStepIndex--; if (currentStepIndex < 0) currentStepIndex += STEPS_PER_REV;
  }
}

static inline int32_t degrees_to_index(int32_t degrees) {
  if (degrees < 0) degrees = ((degrees % 360) + 360) % 360;
  degrees %= 360;
  int32_t idx = (int32_t)((((int64_t)degrees) * STEPS_PER_REV + 180) / 360);
  if (idx >= (int32_t)STEPS_PER_REV) idx = 0;
  return idx;
}

static inline int32_t steps_to_target_dir(int32_t targetIndex, bool dirCCW) {
  if (!dirCCW) { // CW
    int32_t delta = targetIndex - currentStepIndex; if (delta < 0) delta += STEPS_PER_REV; return delta;
  } else {
    int32_t delta = currentStepIndex - targetIndex; if (delta < 0) delta += STEPS_PER_REV; return delta;
  }
}
...
static void handle_command(const String &action, const String &direction, const String &arg1, const String &arg2) {
  // Direction
  if (ieq(direction, "CW")) setDirection(0); else if (ieq(direction, "CCW")) setDirection(1); else {
    Serial.println(F("ERR bad direction (use CW or CCW)")); return;
  }

  if (ieq(action, "ROT")) {
    if (arg1.length() == 0) { Serial.println(F("ERR ROT needs RPM")); return; }
    float rpm = arg1.toFloat(); if (rpm <= 0.0f) { Serial.println(F("ERR RPM must be > 0")); return; }
    stepIntervalTicks = rpm_to_interval_ticks(rpm);
    mode = MODE_ROTATE;
    schedule_next_step_from_now();
    Serial.print(F("OK ROT dir=")); Serial.print(direction); Serial.print(F(" rpm=")); Serial.println(rpm, 3);
  }
  else if (ieq(action, "STP")) {
    if (arg1.length() == 0 || arg2.length() == 0) { Serial.println(F("ERR STP needs steps and ms")); return; }
    long steps = arg1.toInt(); long ms = arg2.toInt();
    if (steps <= 0 || ms <= 0) { Serial.println(F("ERR steps and ms must be > 0")); return; }
    stepsRemaining = steps;
    stepIntervalTicks = ms_to_ticks(ms) / (uint32_t)steps; if (stepIntervalTicks == 0) stepIntervalTicks = 1;
    mode = MODE_STEPS;
    schedule_next_step_from_now();
    Serial.print(F("OK STP dir=")); Serial.print(direction); Serial.print(F(" steps=")); Serial.print(steps);
    Serial.print(F(" ms=")); Serial.println(ms);
  }
  else if (ieq(action, "POS")) {
    if (arg1.length() == 0 || arg2.length() == 0) { Serial.println(F("ERR POS needs deg and ms")); return; }
    long deg = arg1.toInt(); long ms = arg2.toInt(); if (ms <= 0) { Serial.println(F("ERR ms must be > 0")); return; }
    int32_t targetIdx = degrees_to_index((int32_t)deg);
    int32_t stepsNeeded = steps_to_target_dir(targetIdx, dirParam==1);
    if (stepsNeeded == 0) { stop_motion("already-at-position"); return; }
    stepsRemaining = stepsNeeded;
    stepIntervalTicks = ms_to_ticks(ms) / (uint32_t)stepsNeeded; if (stepIntervalTicks == 0) stepIntervalTicks = 1;
    mode = MODE_POSITION;
    schedule_next_step_from_now();
    Serial.print(F("OK POS dir=")); Serial.print(direction); Serial.print(F(" deg=")); Serial.print(deg);
    Serial.print(F(" ms=")); Serial.println(ms);
  }
    ...
    Serial.print(F("MODE="));
    switch (mode) { case MODE_IDLE: Serial.print(F("IDLE")); break; case MODE_ROTATE: Serial.print(F("ROTATE")); break; case MODE_STEPS: Serial.print(F("STEPS")); break; case MODE_POSITION: Serial.print(F("POSITION")); break; }
    Serial.print(F(" dir=")); Serial.print(dirParam==0?F("CW"):F("CCW"));
    Serial.print(F(" nextTick=")); Serial.print(nextStepAtTick);
    Serial.print(F(" intervalTicks=")); Serial.print(stepIntervalTicks);
    Serial.print(F(" remain=")); Serial.print(stepsRemaining);
    Serial.print(F(" idx=")); Serial.print(currentStepIndex);
...
static void read_and_dispatch_serial() {
  while (Serial.available()) {
    char c = (char)Serial.read();
    if (c == '\n' || c == '\r') {
      if (rxLine.length() > 0) {
        String action, direction, a1, a2;
        int i1 = rxLine.indexOf(' ');
        if (i1 < 0) { action = rxLine; direction = ""; }
        else {
          action = rxLine.substring(0, i1);
          int i2 = rxLine.indexOf(' ', i1 + 1);
          if (i2 < 0) { direction = rxLine.substring(i1 + 1); }
          else {
            direction = rxLine.substring(i1 + 1, i2);
            int i3 = rxLine.indexOf(' ', i2 + 1);
            if (i3 < 0) { a1 = rxLine.substring(i2 + 1); }
            else { a1 = rxLine.substring(i2 + 1, i3); a2 = rxLine.substring(i3 + 1); }
          }
        }
        rxLine = ""; action.trim(); direction.trim(); a1.trim(); a2.trim();
        if (action.length() > 0) handle_command(action, direction, a1, a2);
      }
    } else {
      rxLine += c; if (rxLine.length() > 96) rxLine.remove(0, rxLine.length() - 96);
    }
  }
}
...
void setup() {
  Serial.begin(115200);
  while (!Serial) { ; }
  Serial.println(F("TC78H670FTG non-blocking (no library) starting..."));

  // Base pin states
  pinMode(PIN_STBY, OUTPUT); enterStandby();
  pinMode(PIN_EN,   OUTPUT); disableDrv();
...
void loop() {
  ...
  if (tick_fired) {
    ...
    if (!errorOK()) { stop_motion("ERR"); disableDrv(); }
    ...
    if (mode != MODE_IDLE && stepIntervalTicks > 0) {
      while ((int32_t)(now - nextStepAtTick) >= 0) {
        ...
        if (mode == MODE_STEPS || mode == MODE_POSITION) {
          stepsRemaining--; if (stepsRemaining <= 0) { stop_motion("done"); break; }
        }
